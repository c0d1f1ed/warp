...
struct nan_propagate_t {
    static constexpr bool nan_as_missing = false;
};  // current Warp behavior: a<b?a:b (asymmetric in NaN)
struct nan_as_missing_t {
    static constexpr bool nan_as_missing = true;
};  // C fmin/fmax behavior (NaN-as-missing, symmetric)
...
template <typename NanBehavior = nan_propagate_t> inline CUDA_CALLABLE float atomic_min(float* address, float val)
{
    ...
    if (_atomic_min_pre_guard<NanBehavior>(val, __int_as_float(old))) {
        do {
            assumed = old;
...
template <typename NanBehavior = nan_propagate_t> inline CUDA_CALLABLE double atomic_min(double* address, double val)
{
    ...
    if (_atomic_min_pre_guard<NanBehavior>(val, __longlong_as_double(old))) {
        do {
            assumed = old;
...
template <typename NanBehavior = nan_propagate_t> inline CUDA_CALLABLE bfloat16 atomic_min(bfloat16* buf, bfloat16 val)
{
    ...
    bfloat16 old_bf16;
    old_bf16.u = old_val;
    if (_atomic_min_pre_guard<NanBehavior>(val_f, bfloat16_to_float(old_bf16))) {
        do {
            assumed = old_val;
            old_bf16.u = assumed;
...
template <typename NanBehavior = nan_propagate_t> inline CUDA_CALLABLE float atomic_max(float* address, float val)
{
    ...
    if (_atomic_max_pre_guard<NanBehavior>(val, __int_as_float(old))) {
        do {
            assumed = old;
...
template <typename NanBehavior = nan_propagate_t> inline CUDA_CALLABLE double atomic_max(double* address, double val)
{
    ...
    if (_atomic_max_pre_guard<NanBehavior>(val, __longlong_as_double(old))) {
        do {
            assumed = old;
...
template <typename NanBehavior = nan_propagate_t> inline CUDA_CALLABLE bfloat16 atomic_max(bfloat16* buf, bfloat16 val)
{
    ...
    bfloat16 old_bf16;
    old_bf16.u = old_val;
    if (_atomic_max_pre_guard<NanBehavior>(val_f, bfloat16_to_float(old_bf16))) {
        do {
            assumed = old_val;
            old_bf16.u = assumed;
